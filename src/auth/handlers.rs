...
pub async fn register_handler(
    ...
    match users::service::get_user_by_email(&_req.email, _env.db()).await {
        ...
        Ok(existing) => {
            println!(
                "[register_handler] User {} already exists",
                &existing.unwrap().email
            );
            return Ok(warp::reply::json(
                &json!({"status":"error", "message":"Unable to complete registration, email already registered"}),
            ));
        }
        _ => (),
    }
    ...
    match users::service::create_user(_req, _env.db()).await {
        Err(e) => {
            println!(
                "[register_handler] Error registering user {}: {:?}",
                &email, e.message
            );
            return Ok(warp::reply::json(
                &json!({"status":"error", "message":"Registration error"}),
            ));
        }
        _ => {
...
pub async fn login_handler(_req: LoginRequest, _env: Environment) -> WebResult<impl Reply> {
    let user_option = match users::service::get_user_by_email(&_req.email, _env.db()).await {
        Ok(None) => {
            println!(
                "[login_handler] Error authenticating user {:?}",
                &_req.email
            );
        ...
        _ => {
            ...
            return Ok(warp::reply::json(
                &json!({"status":"error", "message":"Email or password unknown"}),
            ));
        }
    };
    ...
    if !is_valid {
        println!(
            "[login_handler] Invalid credentials for user {:?}",
            &_req.email
        );
        return Err(warp::reject::custom(AuthError::InvalidCredentials));
    }
    ...
    println!(
        "[login_handler] Authenticated user '{}' ({})",
