...
pub async fn password_update_handler(
    mut _req: PasswordUpdateRequest,
    _env: Environment,
    _user: AuthUser,
) -> WebResult<impl Reply> {
    ...
    println!(
        "[password_update_handler] Action performed by {} on {}",
        _user.id, user.id
    );
    // current_password is required for users/admins to change their own passwords, but allow admins change others'
    if (_user.id != user.id.to_string() && _user.role != Role::Admin)
        || _user.id == user.id.to_string()
    {
        let is_valid = _env
            .argon()
            .verifier()
    ...
    let hash = _env
        .argon()
        .hasher()
        .with_password(&_req.new_password)
        .hash()
        .unwrap();
    user.password = hash;
    service::update_user_password(user, _env.db())
        .await
        .map(|_e| UserError::UpdateError);
    Ok(warp::reply::json(
