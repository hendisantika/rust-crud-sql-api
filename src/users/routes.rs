...
pub fn routes(_env: Environment) -> BoxedFilter<(impl Reply,)> {
    let get_users_route = warp::get().and(
        warp::path!("api" / "users")
    ...
    let get_user_route = warp::get().and(
    ...
    let user_create_route = warp::post().and(
        warp::path!("api" / "users")
            .and(warp::body::json())
            .and(environment::with_env(_env.clone()))
            .and(auth::middleware::with_auth(Role::Admin))
    ...
    let user_update_route = warp::put().and(
        warp::path!("api" / "users")
            .and(warp::body::json())
            .and(environment::with_env(_env.clone()))
            .and(auth::middleware::with_auth(Role::Admin))
    ...
    let user_password_update_route = warp::put().and(
        warp::path!("api" / "users" / "changePassword")
            .and(warp::body::json())
            .and(environment::with_env(_env.clone()))
