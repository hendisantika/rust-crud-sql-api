...
pub async fn update_article_handler(
    ...
    let mut original = service::get_article_by_id(_req.id.unwrap(), _env.db())
        .await?
        .unwrap();
...
pub async fn delete_article_handler(
    ...
    Ok(warp::reply::json(
        &json!({"status":"success", "message":"Article deleted"}),
    ))
}
...
pub async fn post_comment_handler(mut _req: Comment, _env: Environment) -> WebResult<impl Reply> {
    ...
    println!(
        ...
        _req.article_id, _req.email, _req.author
    );
    Ok(warp::reply::json(
        &json!({"status":"success", "message":"Comment saved"}),
    ))
}
